...
public class SearchCategoryQ extends AppCompatActivity {
    ListView listView;
    String[] title;
    String[] description;
    int[] icon;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < title.length; i++) {
            ...
            //bind all strings in an array that contain the word fast (needs to be adjusted to our database)
            if(title[i].contains("fast") || title[i].contains("Fast") || description[i].contains("fast") || description[i].contains("Fast")) {
