...
public class SearchCategoryO extends AppCompatActivity {
    ListView listView;
    String[] title;
    String[] description;
    int[] icon;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < title.length; i++) {
            ...
            //bind all strings in an array that contain the word healthy (needs to be adjusted to our database)
            if(!(title[i].contains("fast") || title[i].contains("Fast") || description[i].contains("fast") ||
                    description[i].contains("Fast") || title[i].contains("healthy") || title[i].contains("Healthy")
                    || description[i].contains("healthy") || description[i].contains("Healthy") ||
                    title[i].contains("vegetarian") || title[i].contains("Vegetarian") || description[i].contains("vegetarian")
                    || description[i].contains("Vegetarian"))) {
